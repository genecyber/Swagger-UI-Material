...
SwaggerUi.Views.ParameterView = Backbone.View.extend({
  ...
  render: function() {
    ...
    if (sampleJSON) {
      ...
      $('.model-signature', $(this.el)).html("<span class='propType'>" + this.model.signature + "</span>");
    }

    var isParam = false;
    
    if( window.ace && this.model.isBody && this.model.schema) {
      var $self = $(this.el);
      
      this.model.editor = ace.edit($('.editor_holder', $self)[0]);
      this.model.editor.renderer.setOption("showPrintMargin", false);
      this.model.editor.session.setMode("ace/mode/json");

      signatureModel.editor = this.model.editor;

      $('.body-textarea', $self).hide();
      $('.editor_holder', $self).show().addClass("loaded");
      $('.parameter-content-type', $self).change(function(e){
        if(e.target.value === 'application/xml') {
          this.model.editor.session.setMode("ace/mode/xml");
        }else{
          this.model.editor.session.setMode("ace/mode/json");
        }
      }.bind(this));
