...
SwaggerUi.Views.OperationView = Backbone.View.extend({
  ...
  initialize: function(opts) {
    ...
    this.auths = this.options.auths;
    this.parentId = this.model.parentId;
    this.nickname = this.model.nickname;
    this.model.encodedParentId = encodeURIComponent(this.parentId);

    if (this.options.swaggerOptions) {
      this.model.defaultRendering = this.options.swaggerOptions.defaultModelRendering;

      if (this.options.swaggerOptions.showRequestHeaders) {
  ...
  render: function() {
    ...
    if (signatureModel) {
      ...
      $('.model-signature', $(this.el)).html("<span class='propType'>" + this.model.type + "</span>");
    }
    contentTypeModel = {
      isParam: false
    };
  ...
  submitOperation: function(e) {
    ...
    if (error_free) {
      ...
      // Assign json editor fields
      for(var pi = 0; pi < this.model.parameters.length; pi++) {
        var p = this.model.parameters[pi];
        if(p.editor) {
          var json = p.editor.getValue();
          map[p.name] = json;
        }
      }
